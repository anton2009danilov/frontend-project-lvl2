...
const resultComplexPath = getFixturePath('result_complex.txt');
...
const resultComplex = readFileSync(resultComplexPath, 'utf-8', (err, data) => {
  if (err) throw err;
  return JSON.parse(data);
});
...
test('gendiff', () => {
  ...
  const fixture1 = getFixturePath('file1_complex.json');
  const fixture2 = getFixturePath('file2_complex.json');
  // eslint-disable-next-line
  expect(action(fixture1, fixture2)).toBe(resultComplex);
});
